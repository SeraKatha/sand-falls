...
fn render_chunk<'a>(canvas: &mut Canvas, chunk: ChunkView<'a, Cell>) {
    let chunk_index = chunk.get_chunk_index();
    let chunk_coord = chunk.get_chunk_coord();
    let cells = chunk.get_cells();
    let texture = &mut canvas.get_textures()[chunk_index];
...
pub struct SingleColorRenderer {
    canvas: Canvas,
}
...
impl super::Renderer for SingleColorRenderer {
    ...
    fn render(&mut self, simulation: &Simulation) {
        ...
        for chunk_index in 0..num_of_chunks_total {
            ...
            render_chunk(&mut self.canvas, chunk);
        }
    }
}
